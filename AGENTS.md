...
- **意图自动驱动**: 识别到特定业务意图时，必须优先触发对应 MCP 工具（如数据库查询对应 `dbhub`），严禁在工具可用时通过代码逻辑推测数据状态。
...
- **🛑 dbhub (数据库操作 - 强制只读)**:
    - **自动触发**: 凡涉及查看表结构 (Schema)、数据库结构或数据验证，**必须自动且优先**调用 `dbhub` 相关工具，严禁仅凭代码推测。**推荐优先使用 `SHOW CREATE TABLE <table_name>` 获取最完整的结构定义。**
